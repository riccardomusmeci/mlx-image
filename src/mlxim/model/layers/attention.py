...
class LinearKMaskedBias(nn.Linear):
    """Linear layer with masked bias for K projection."""
...
class RoPESelfAttention(nn.Module):
    ...
    def apply_rope(self, q: mx.array, k: mx.array, rope: tuple[mx.array, mx.array]) -> tuple[mx.array, mx.array]:
        q_dtype = q.dtype
        k_dtype = k.dtype
        sin, cos = rope
        ...
        q = mx.concat([q_prefix, q], axis=-2)

        k_prefix = k[:, :, :prefix, :]
        k = rope_apply(k[:, :, prefix:, :], sin, cos)
        k = mx.concat([k_prefix, k], axis=-2)

        return q.astype(q_dtype), k.astype(k_dtype)

    def __call__(
        self, x: mx.array, attn_bias: mx.array | None = None, rope: tuple[mx.array, mx.array] | None = None
    ) -> mx.array:
        qkv = self.qkv(x)
    ...
    def compute_attention(
        self, qkv: mx.array, attn_bias: mx.array | None = None, rope: tuple[mx.array, mx.array] | None = None
    ) -> mx.array:
